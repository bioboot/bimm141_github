...
Write a function to generate random nucleotide (and protein) sequences of a user specified length.

```{r}
generate_dna_sequence <- function(length) {

  # Create a vector of all possible bases
  bases <- c("A", "T", "C", "G")
...
generate_protein_sequence <- function(length, single=TRUE) {
  ...
  if(single) {
    # Return sequence as a single element vector
...
```{r}
generate_protein_sequence(10)
generate_protein_sequence(6)
```
Blast search these sequences to see if we find matches. But first lets generate sequences of length 6 to 12:

```{r}
seqs <- sapply(6:12, generate_protein_sequence)
```
